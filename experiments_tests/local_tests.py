...
def constrained_test():
    ...
    Thread(target=PROCS["0"].agree_on_value, args=[dates_to_unix(proc0_blocks)]).start()
    Thread(target=PROCS["1"].agree_on_value, args=[dates_to_unix(proc1_blocks)]).start()
    Thread(target=PROCS["2"].agree_on_value, args=[dates_to_unix(proc2_blocks)]).start()
